...
I also estimate the effects of just events related to sexual assault allegations in Figure~\ref{figure:events_police_allegations}, and the effects of 'big allegations' in Figure~\ref{figure:events_police_big}. Both of these figures are in 3-day bins to get consistent effect estimates.

\begin{figure}
\includegraphics[width=\linewidth]{figures/events_police_allegations.eps}
\caption{Reports to the FBI before and after allegations of sexual assault}\label{figure:events_police_allegations}
\end{figure}

\begin{figure}
\includegraphics[width=\linewidth]{figures/events_police_big.eps}
...
\begin{itemize}
    \item Break down by race and alc
    \begin{itemize}
        \item Done in clean raw, needs to be run, then put into analysis
    \end{itemize}
    \item Think through binning. How do I interpret? Am I doing it right?
    \item Write down IV arrows, think about how to run
    \item If makes sense, merge police, trend and events into one
    \item Merge T9 investigations into police data by county and state (have this code approx higher up)
    \item Run police daily event studies within county/state after T9 case is opened
    \item Do T9 case nationally, no effect?
    \item Go over clean\_raw and get population correct. Change main graph of reports to per capita. Check how much it's changing over time. Think about doing per cap and weighting.
    \item Show line graph of Bill Cosby with fixed effects? possible?, extrapolate how many reports it increased approx. UNSURE ON THIS
    \item Talk to PB about methodology on trend. Maybe just use original trend? Check only over time period I'm using too.
    \item Calculate average event effect, per cap? make equivalent to the US. Note the assumption in homogeneous effect.
    ...
    \item Think about how this may be people reporting more quickly - 
